...
def imaging_ca_oscillation_analysis(adquisiton_time, keyword, peak_amplitude, peak_longitude, route, time_initial_linregress, time_final_linregress, y_max_value, y_min_value):
    ...
    for sheet_name in sheets:
        ...
        for column in y.columns:
            ...
            append_df_to_excel(file_route, df_peaks.T, sheet_name=sheet_name,
            ...
            df_max = pd.DataFrame(
                {parameter_value_max: properties["prominences"]})
            append_df_to_excel(file_route, df_max.T, sheet_name=sheet_name,
            ...
            append_df_to_excel(file_route, df_width.T, sheet_name=sheet_name,
                               startrow=row_num_data, startcol=column_num_data, header=False, index=True, truncate_sheet=False)
            row_num_data += 2
        ...
        print("")
        print(
            f"Number of analyzed cells: {ca_cal.analyzed_cell_number(sheet)}.")
                        ...
                        for column in range(1, final_file_max_column):
                            if sheet.cell(row=row, column=column).value:
                                ...
                                if type(sheet[float_formating].value) == float:
                                    ca_ex_st.style_number(
                                        float_formating, sheet)
...
peak_longitude = 1
route = Path(
    r"H:\20220526_Airyscan\Mam_231_WT_ca_histamina - Z=0_imageJ_F_F0.xlsx")
time_initial_linregress = 2
time_final_linregress = 2
y_max_value = ""
y_min_value = ""

imaging_ca_oscillation_analysis(adquisiton_time, keyword, peak_amplitude, peak_longitude,
                                route, time_initial_linregress, time_final_linregress, y_max_value, y_min_value)
