...
def imaging_ca_oscillation_analysis(adquisiton_time, peak_amplitude, peak_longitude, route, time_initial_linregress, time_final_linregress):
    ...
    for column in range(1, file_max_column + 1):
        try:
            if "Ratio" in sheet.cell(row=1, column=column).value:
                sample_number_header = sheet.cell(
                    row=1, column=column).coordinate
                sheet[sample_number_header] = f"#{number_cell:02} Ratio"
                number_cell += 1
            elif "FITC" in sheet.cell(row=1, column=column).value:
                sample_number_header = sheet.cell(
                    row=1, column=column).coordinate
                sheet[sample_number_header] = f"#{number_cell:02} FITC"
                number_cell += 1
        except TypeError:
            pass
    ...
    print(f"{non_space_filename}_modified.xlsx has been saved.")
    ...
    for column in y.columns:
        ...
        splitted_file = file_route.split(".xlsx")[0]
        temporal_images = os.path.join(converted_route, splitted_file)
        ...
        for subfolder in subfolders:
            if subfolder == splitted_file:
                ...
                for temporal_image in os.listdir(subfolder):
                    ...
                    sheet.add_image(img)
                    row_num_plots += 26
                filename = f"{splitted_file}_analyzed.xlsx"

                finalfile_max_row = sheet.max_row
                final_file_max_column = sheet.max_column

                try:
                    for row in range(1, finalfile_max_row + 1):
                ...
                for row in range(1, finalfile_max_row + 1):
                    try:
                        ...
                        pass
                ca_ex_st.automatic_column_widths(sheet)

                wb.save(filename)
                print(f"{filename} has been saved.")


def imaging_ca_oscillation_multi_analysis(adquisiton_time, file, folder, peak_amplitude, peak_longitude, route, time_initial_linregress, time_final_linregress):
    ...
    modified_file = f"{splitted_file}_modified.xlsx"
    save_route = os.path.join(folder, modified_file)
    wb.save(save_route)
    print(f"{modified_file} has been saved.")

    for folders, _, files in os.walk(folder):
        for file in files:
            if "_modified" in file:
                ...
                for column in y.columns:
                    ...
                    addapted_df_to_excel(column, list_peaks, file_route,
                                         column_num_data, row_num_data, "Peak time")
                    row_num_data += 1
                    peaks, properties = find_peaks(
                        y[column], prominence=peak_amplitude, width=peak_longitude)
                    properties["prominences"], properties["widths"]
                    list_max = list(properties["prominences"])
                    addapted_df_to_excel(column, list_max, file_route,
                                         column_num_data, row_num_data, "Max")
                    row_num_data += 1
                    list_width = list(properties["widths"])
                    addapted_df_to_excel(column, list_width, file_route,
                                         column_num_data, row_num_data, "Width")
                    row_num_data += 2
                    ...
                    ax[0].fill_between(
                        x, y[column], df_initial_linear_regresion, color="peachpuff", where=x > 28)
                    ax[1].fill_between(
                        x, y[column], df_final_linear_regresion, color="wheat", where=x > 28)
                    ax[0].xaxis.set_major_locator(MultipleLocator(60))
                    ax[0].xaxis.set_minor_locator(MultipleLocator(30))
                    ax[1].xaxis.set_major_locator(MultipleLocator(60))
                    ax[1].xaxis.set_minor_locator(MultipleLocator(30))
                    ...
                    splitted_file = file.split(".xlsx")[0]
                    temporal_images = os.path.join(folder, splitted_file)
                    Path(temporal_images).mkdir(parents=True, exist_ok=True)
                    fig_name = os.path.join(temporal_images, f"{column}.png")
                    plt.savefig(fig_name, dpi=100)
                    # plt.show()
    ...
    for carpeta, subfolders, files in os.walk(folder):
        for subfolder in subfolders:
            for file in files:
                analyzed_file = modified_file.split(".xlsx")[0]
                if subfolder == analyzed_file:
                    analyzed_file_path = os.path.join(carpeta, modified_file)
                    wb = openpyxl.load_workbook(analyzed_file_path)
                    sheet = wb.active
                    for temporal_image in os.listdir(os.path.join(carpeta, subfolder)):
                        temporal_image_route = os.path.join(
                            carpeta, subfolder, temporal_image)
                        img = openpyxl.drawing.image.Image(
                            temporal_image_route)
                        img_cell = sheet.cell(
                            row=row_num_plots, column=column_num_plots).coordinate
                    ...
                    for row in range(1, file_max_row + 1):
                        try:
                            parameter_sample = sheet.cell(
                                row=row, column=column_num_data).coordinate
                            ca_ex_st.style_oscillation_calculated_parameters_header(
                                parameter_sample, sheet)
                        except TypeError:
                            pass
                    filename = f"{analyzed_file}_analyzed.xlsx"
                    wb.save(filename)
