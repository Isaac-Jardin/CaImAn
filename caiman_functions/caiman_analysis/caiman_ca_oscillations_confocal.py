...
def confocal_ca_oscillation_analysis(adquisiton_time, peak_amplitude, peak_longitude, route, time_initial_linregress, time_final_linregress):
    ...
    for column in range(1, file_max_column + 1):
        try:
            if "FITC" in sheet.cell(row=1, column=column).value:
    ...
    wb.save(f"{route.stem}_modified.xlsx")
    print(f"{route.stem}_modified.xlsx has been saved.")

    file_route = f"{route.stem}_modified.xlsx"
