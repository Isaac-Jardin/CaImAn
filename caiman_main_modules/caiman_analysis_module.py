...
from caiman_functions.caiman_analysis.caiman_ca_oscillations_PCA import imaging_ca_analysis_PCA, imaging_ca_analysis_PCA_multiple
...
def complete_analysis_module():
    while True:
        ...
        if analysis_study == "SOCE analysis (single file).":
        ...
        elif analysis_study == "Imaging calcium analysis PCA multiple":
            route_input = pyip.inputFilepath(
                prompt="Please enter your folder path: ")
            ...
            peak_longitude = pyip.inputFloat(
                prompt="Introduce your sought peak longitude (width) or leave it empty to set the default (10): ", blank=True)
            if peak_longitude == "":
                peak_longitude = 10
            ...
            start_time = time.time()
            imaging_ca_analysis_PCA_multiple(adquisiton_time, keyword, peak_amplitude,
                                             peak_longitude, route_input, time_initial_linregress, y_max_value, y_min_value)
            print(
                f"Execution time: {round((time.time()-start_time), 2)} seconds.")
            print("Done.\n")
